    ...
    <el-form ref="form" :rules="rules" :model="form" class="form">
      ...
      <div class="form_item">
        <el-form-item prop="verifyId" :rules="[{ required: true, message: '請輸入個人帳號', trigger: 'blur' }]">
          ...
          <el-input id="verifyId" v-model="form.verifyId" name="verifyId" :disabled="editAcct==true?true:false" />
        </el-form-item>
        <el-form-item>
      ...
      <el-form-item v-if="editAcct" style="width: 600px">
...
export default {
  ...
  methods: {
    editAcctForm () {
      ...
      vm.thisAcctType = localStorage.getItem('TSLtype')
      if (vm.closeDiv(id) !== false) {
        vm.editAcct = true
        vm.editForm(id)
      } else {
        vm.editAcct = false
        if (vm.closeDiv(vm.$route.query.acctType) !== false) {
          vm.acctTypeId = vm.$route.query.acctType
        }
      }
    },
    async editForm (id) {
      ...
      this.$refs[formName].validate(async (valid) => {
        if (valid) {
          ...
          const obj = {
            ...
            accountNum: '',
            allowSupport: '',
            acctStatus: vm.thisAcctType === '5' ? '3' : '1',
            files: '',
            isReal: vm.thisAcctType === '5' ? '1' : '0',
            remark: form.remark
          }
