...
<template>
  <div class="newsform_edit">
    <span class="breadcrumb">
      <a id="a_c" href="#a_c" accesskey="C" class="breadcrumb_title" title="目前位置">
        :::目前位置：
      </a>
      <a href="/" title="首頁">首頁</a> / <a href="/news">最新消息</a> / <a href="/news/editForm">編輯最新消息</a>
    </span>

    <div id="content" class="newsform_edit--title">
    ...
    <el-form ref="form" :model="form" class="form">
      <div class="form_item sone">
        <el-form-item prop="title" :rules="[{ required: true, message: '請輸入標題', trigger: 'blur' }]">
      ...
      <div class="referLink">
        <p>參考連結 (上限五個超連結)</p>
        <div v-for="item,index in form.newsReferenceLinks" :key="item.id" class="referLink_link">
          ...
          <div class="referLink_icon">
            <a v-if="index != 0" href="javascript:void(0);" class="referLink_delete" @click="removeArr(form.newsReferenceLinks,item)">
              <img src="~/static/img/icon/delete.svg" alt="刪除參考連結">
            </a>
            <a v-if="form.newsReferenceLinks.length == (index+1) && form.newsReferenceLinks.length < 5" href="javascript:void(0);" class="referLink_add" @click="addreferLink">
...
export default {
  ...
  head: {
    title: '新北市手語翻譯暨同步聽打服務線上申辦系統-最新消息-編輯最新消息'
  },
  mounted () {
    this.getForm()
    this.getacctform()
  ...
  methods: {
    getForm () {
      const vm = this
      const form = JSON.parse(sessionStorage.getItem('newsForm'))
      vm.form.gid = form.gid
      vm.form.seqNum = form.seqNum
      vm.form.title = form.title
      vm.form.beginDateTime = vm.$moment(form.beginDateTime).format('yyyy/MM/DD')
      vm.form.endDateTime = vm.$moment(form.endDateTime).format('yyyy/MM/DD')
      vm.form.contant = form.contant
      vm.form.contactName = form.contactName
      vm.form.contactGroup = form.contactGroup
      vm.form.contactPhoneNum = form.contactPhoneNum
      vm.form.email = form.email
      if (form.newsReferenceLinks.length !== 0) {
        vm.form.newsReferenceLinks = form.newsReferenceLinks
      }
      if (form.files.length !== 0) {
        for (let i = 0; i < form.files.length; i++) {
          const item = form.files[i]
    ...
    async submitForm (vForm, fileToList) {
      ...
      const obj = {
        ...
        updatedById: vm.verifyId,
        contactName: form.contactName,
        contactPhoneNum: form.contactPhoneNum,
        contactGroup: form.contactGroup,
        email: form.email
      }
      const res = await frontendApi.edit_news(obj)
      if (res.isSuccess === true) {
        if (fileToList === true) {
          this.UpdateForeignKey(form.gid)
        } else {
          this.loading = false
          vm.defaultText = '編輯消息成功'
          vm.defaultId = 1
